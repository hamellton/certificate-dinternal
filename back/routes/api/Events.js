...
router.get('/code/:code', (req, res) => {
  ...
  const getEvents = async () => {
    ...
    const typeNum = Number(codeArr[1]);
    const date = `${codeArr[2].slice(0, 2)}.${codeArr[2].slice(2, 4)}.${codeArr[2].slice(4, 8)}`;
    const time = codeArr[2].slice(8);
    ...
    events = events.filter((e) => e.type_num === typeNum && e.date === date && e.time === time);
    events.map((item) => {
    ...
    if (events[0]) {
      res.send(events);
      log.info('event by code has been send');
    } else {
      log.info('event by code ERROR');
      res.send(false);
    }
  };
  getEvents();
